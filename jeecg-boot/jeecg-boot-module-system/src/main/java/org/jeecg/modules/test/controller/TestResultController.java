...
@Api(tags="测试结果")
@RestController
@RequestMapping("/test/testResult")
public class TestResultController {
	...
	public Result<IPage<TestResult>> queryPageList(TestResult testResult,
		...
		Result<IPage<TestResult>> result = new Result<IPage<TestResult>>();
	...
	public Result<TestResult> add(@RequestBody TestResult testResult) {
		Result<TestResult> result = new Result<TestResult>();
		try {
	...
	public Result<TestResult> edit(@RequestBody TestResult testResult) {
		Result<TestResult> result = new Result<TestResult>();
		TestResult testResultEntity = testResultService.getById(testResult.getId());
		if(testResultEntity==null) {
			result.error500("未找到对应实体");
		}else {
			boolean ok = testResultService.updateById(testResult);
			//TODO 返回false说明什么？
			if(ok) {
				result.success("修改成功!");
			}
	...
	public Result<TestResult> deleteBatch(@RequestParam(name="ids",required=true) String ids) {
		Result<TestResult> result = new Result<TestResult>();
  ...
  public ModelAndView exportXls(HttpServletRequest request, HttpServletResponse response) {
      // Step.1 组装查询条件
      QueryWrapper<TestResult> queryWrapper = null;
      try {
          String paramsStr = request.getParameter("paramsStr");
          if (oConvertUtils.isNotEmpty(paramsStr)) {
              String deString = URLDecoder.decode(paramsStr, "UTF-8");
              TestResult testResult = JSON.parseObject(deString, TestResult.class);
              queryWrapper = QueryGenerator.initQueryWrapper(testResult, request.getParameterMap());
          }
      ...
      List<TestResult> pageList = testResultService.list(queryWrapper);
      //导出文件名称
      mv.addObject(NormalExcelConstants.FILE_NAME, "测试结果列表");
      mv.addObject(NormalExcelConstants.CLASS, TestResult.class);
      mv.addObject(NormalExcelConstants.PARAMS, new ExportParams("测试结果列表数据", "导出人:Jeecg", "导出信息"));
  ...
  public Result<?> importExcel(HttpServletRequest request, HttpServletResponse response) {
      ...
      for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
          ...
          try {
              List<TestResult> listTestResults = ExcelImportUtil.importExcel(file.getInputStream(), TestResult.class, params);
              testResultService.saveBatch(listTestResults);
              return Result.ok("文件导入成功！数据行数:" + listTestResults.size());
          } catch (Exception e) {
              log.error(e.getMessage(),e);
